...
public class Ex11_62 {
	...
	static void inputRecord() {		
		...
		while(true) {
			...
			try {
				...
				} catch (Exception e) { System.out.println("입력오류입니다. 이름, 학번, 국어성적, 영어성적" + ", 수학성적'의 순서로 입력하세요.");
			}		
		}
	}
	...
	static void deleteRecord() {
		while(true) {
			...
			} catch (Exception e) { System.out.println("입력오류입니다. 다시 입력해 주세요.");}
		}
	}
	...
	static void displayRecord() {
		...
		System.out.println("=================================");
		System.out.println("총점: " + Student2.format(koreanTotal+"",6,Student2.RIGHT)
...
class Student2 implements Comparable {
	...
	public String toString() {
		return format(name,4,LEFT) + format(studentNo,4,RIGHT) 
			+ format(""+koreanScore,6,RIGHT) + format(""+mathScore,6,RIGHT)
			+ format(""+englishScore,6,RIGHT) + format(""+total,8,RIGHT);
	}
	...
	static String format(String str, int length, int alignment) {
		...
		switch(alignment) {
			case CENTER : System.arraycopy(source, 0, result, diff/2, source.length); break;
			case RIGHT : System.arraycopy(source, 0, result, diff, source.length); break;
			case LEFT : default : System.arraycopy(source, 0, result, 0, source.length);
		}
